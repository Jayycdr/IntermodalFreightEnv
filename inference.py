...
def run_episode(task_type: str = "task_1_time", episode_num: int = 1, max_steps: int = 5) -> float:
    ...
    try:
        ...
        # Normalize score to (0, 1) exclusive - clamp to (0.01, 0.99) which survives .2f formatting
        final_score = max(min(float(final_score), 0.99), 0.01)
        ...
        final_score = 0.01  # Use 0.01 to satisfy strict (0,1) requirement and survive .2f formatting
        success = False
        rewards_list = []
        steps_taken = 0
